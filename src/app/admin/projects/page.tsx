...
import { FolderOpen, Calendar, User, Target, DollarSign, CheckCircle } from 'lucide-react';
...
export default function AdminProjectsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (companiesRes.success) {
        const companiesData = companiesRes.items || companiesRes.data || [];
        setCompanies(Array.isArray(companiesData) ? companiesData as CompanyData[] : []);
      }
    } catch (error) {
      console.error('Error fetching data:', error);
      setProjects([]); // Set empty arrays if API fails
      setCompanies([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateProject = async () => {
    try {
      const response = await ProjectApiService.createProject(formData as ProjectData);
      if (response.success) {
  ...
  const handleEditProject = async () => {
    ...
    try {
      const response = await ProjectApiService.updateProject(editingProject.id, formData);
      if (response.success) {
        setShowCreateModal(false);
